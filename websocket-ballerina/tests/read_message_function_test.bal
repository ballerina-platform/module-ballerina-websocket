...
public function testSyncClientReadMessage() returns Error? {
   ...
   worker w1 {
      io:println("Reading message starting: sync read message client");

      byte[]|string|Error resp1 = wsClient->readMessage();
      if (resp1 is string) {
         readMessageStringOutput = readMessageStringOutput + resp1;
         io:println("1st response received at sync read message client :" + resp1);
      }

      byte[]|string|Error resp2 = checkpanic wsClient->readMessage();
      if (resp2 is string) {
         readMessageStringOutput = readMessageStringOutput + resp2;
         io:println("2nd response received at sync read message client :" + resp2);
      } else if (resp2 is byte[]) {
         io:println("2nd response received at sync read message client :" + resp2.toString());
         readMessageByteOutput = resp2.toString();
      }

      byte[]|string|Error resp3 = checkpanic wsClient->readMessage();
      if (resp3 is string) {
         readMessageStringOutput = readMessageStringOutput + resp3;
         io:println("3rd response received at sync read message client :" + resp3);
      }

      runtime:sleep(3);

      byte[]|string|Error resp4 = checkpanic wsClient->readMessage();
      if (resp4 is string) {
         readMessageStringOutput = readMessageStringOutput + resp4;
         io:println("4th response received at sync read message client :" + resp4);
      } else if (resp4 is byte[]) {
         io:println("4th response received at sync read message client :" + resp4.toString());
         readMessageByteOutput = readMessageByteOutput + resp4.toString();
      }

      byte[]|string|Error resp5 = checkpanic wsClient->readTextMessage();
      if (resp5 is string) {
         readMessageStringOutput = readMessageStringOutput + resp5;
         io:println("Final response received at sync read message client :" + resp5);
      }
   }
   @strand {
      thread:"any"
