...
service pushTextFailureService on new websocket:Listener(21008) {
    resource function onOpen(websocket:WebSocketCaller caller) {
        websocket:WebSocketError? err1 = caller->close(timeoutInSeconds = 0);
...
public function pushTextFailure() {
    websocket:WebSocketClient wsClient = new ("ws://localhost:21008/pushTextFailureService");
    runtime:sleep(500);
