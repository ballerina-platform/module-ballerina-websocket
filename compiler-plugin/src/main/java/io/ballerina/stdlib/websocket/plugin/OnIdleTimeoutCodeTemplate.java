...
/**
 * Code action to add onIdletimeout remote function.
 */
public class OnIdleTimeoutCodeTemplate implements CodeAction {
    ...
    public List<String> supportedDiagnosticCodes() {
        return List.of(PluginConstants.CompilationErrors.ON_IDLE_TIMEOUT_GENERATION_HINT.getErrorCode());
    }

    @Override
    public Optional<CodeActionInfo> codeActionInfo(CodeActionContext codeActionContext) {
        return Utils.getCodeActionInfo(codeActionContext, "Add onIdleTimeout function");
    }

    @Override
    public List<DocumentEdit> execute(CodeActionExecutionContext codeActionExecutionContext) {
        return Utils.getDocumentEdits(codeActionExecutionContext, ON_IDLE_TIMEOUT_FUNCTION);
    }

    @Override
    public String name() {
        return "ADD_ON_IDLE_TIMEOUT_CODE_SNIPPET";
    }
}
