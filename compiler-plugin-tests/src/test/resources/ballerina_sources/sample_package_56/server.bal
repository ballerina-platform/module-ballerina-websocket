...
service class WsService {
    *websocket:Service;
    ...
    remote isolated function onOpen(websocket:Caller caller) returns stream<string>|error {
        string[] greets = ["Hi Sam", "Hey Sam", "GM Sam"];
        return greets.toStream();
    }

    remote isolated function onMessage(json data) returns stream<string, error?>|error {
        string[] greets = ["Hi Sam", "Hey Sam", "GM Sam"];
        return greets.toStream();
    }
}
