...
The received text messages are dispatched to this remote method. Data binding support is provided to accept the text messages as `json` and its subtypes, `xml`, `record{}` and `record{}[]`.
...
`writeTextMessage` API can be used to send a text message. It takes in the message to be sent as a `json` and its subtypes(`string`, `int`, `float`, `decimal`, `boolean`), `xml`, `record{}`, `record{}[]` and returns an error if an error occurs while sending the text message to the connection.
...
`readTextMessage` API can be used to receive a text message. It returns the complete text message as one of `json` and its subtypes, `xml`, `record{}` and `record{}[]` or else an error if an error occurs while reading the messages. The contextually-expected data type is inferred from the LHS variable type.
